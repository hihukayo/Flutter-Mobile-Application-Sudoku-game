...
- **用户系统**：注册 / 登录 / 注销，修改用户名 / 手机号 / 密码；同一设备缓存最近一次成功登录（账号 + 密码），输入一致即可离线直登
...
- **云存档（本地 + 云端同步）**：存档先写入本地 SQLite（离线也能存），再后台同步云端；读档合并本地与云端、按保存时间取较新的一份（云端新则覆盖本地，本地新则上传覆盖云端）；手动保存 / 加载 + 自动存档，未游玩的新盘不会覆盖旧存档，读档后即视为已玩过，暂停 / 退出自动保存，计时回溯到存档时刻
...
<table align="center" width="640">
  ...
  <tr>
    <td width="320" align="center">本地存储</td>
    <td width="320" align="center">SQLite (sqflite)</td>
  </tr>
  <tr>
...
- **同一 WiFi**：手机与电脑连同一热点，电脑运行 `ipconfig` 查看局域网 IP（如 `192.168.43.74`），App 设置 → 服务器地址 填写 `192.168.43.74:8080`（普通请求 4 秒、存档 / 读档 3 秒超时，超时可重试）
...
│   ├── services/api_service.dart         # API 请求封装（可配置服务器地址，同步 3 秒 / 普通 4 秒超时）
│   └── services/local_save_store.dart    # 本地 SQLite 存档（离线存 / 读 + 云端同步标记）
├── assets/audio/                  # 音效资源
├── android/                       # Android 工程
├── ios/                           # iOS 工程
