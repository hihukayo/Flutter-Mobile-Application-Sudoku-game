import 'dart:convert';
import 'dart:typed_data';

/// 纯 Dart SHA-256 实现（谜题指纹用，避免额外依赖）
Uint8List sha256Bytes(List<int> data) {
  final k = <int>[
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1,
    0x923f82a4, 0xab1c5ed5, 0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
    0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174, 0xe49b69c1, 0xefbe4786,
    0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147,
    0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
    0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85, 0xa2bfe8a1, 0xa81a664b,
    0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a,
    0x5b9cca4f, 0x682e6ff3, 0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
    0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
  ];
  int rotr(int x, int n) => ((x >> n) | (x << (32 - n))) & 0xffffffff;

  final bitLen = data.length * 8;
  final paddedLen = ((data.length + 9 + 63) ~/ 64) * 64;
  final padded = Uint8List(paddedLen);
  padded.setAll(0, data);
  padded[data.length] = 0x80;
  final end = paddedLen - 8;
  for (var i = 0; i < 8; i++) {
    padded[end + i] = (bitLen >> (56 - i * 8)) & 0xff;
  }

  var a = 0x6a09e667, b = 0xbb67ae85, c = 0x3c6ef372, d = 0xa54ff53a;
  var e = 0x510e527f, f = 0x9b05688c, g = 0x1f83d9ab, h = 0x5be0cd19;

  final w = List<int>.filled(64, 0);
  for (var chunk = 0; chunk < paddedLen; chunk += 64) {
    for (var i = 0; i < 16; i++) {
      w[i] = (padded[chunk + i * 4] << 24) |
          (padded[chunk + i * 4 + 1] << 16) |
          (padded[chunk + i * 4 + 2] << 8) |
          padded[chunk + i * 4 + 3];
    }
    for (var i = 16; i < 64; i++) {
      final s0 = rotr(w[i - 15], 7) ^ rotr(w[i - 15], 18) ^ (w[i - 15] >> 3);
      final s1 = rotr(w[i - 2], 17) ^ rotr(w[i - 2], 19) ^ (w[i - 2] >> 10);
      w[i] = (w[i - 16] + s0 + w[i - 7] + s1) & 0xffffffff;
    }
    var aa = a, bb = b, cc = c, dd = d;
    var ee = e, ff = f, gg = g, hh = h;
    for (var i = 0; i < 64; i++) {
      final s1 = rotr(ee, 6) ^ rotr(ee, 11) ^ rotr(ee, 25);
      final ch = (ee & ff) ^ (~ee & gg);
      final t1 = (hh + s1 + ch + k[i] + w[i]) & 0xffffffff;
      final s0 = rotr(aa, 2) ^ rotr(aa, 13) ^ rotr(aa, 22);
      final maj = (aa & bb) ^ (aa & cc) ^ (bb & cc);
      final t2 = (s0 + maj) & 0xffffffff;
      hh = gg; gg = ff; ff = ee; ee = (dd + t1) & 0xffffffff;
      dd = cc; cc = bb; bb = aa; aa = (t1 + t2) & 0xffffffff;
    }
    a = (a + aa) & 0xffffffff; b = (b + bb) & 0xffffffff;
    c = (c + cc) & 0xffffffff; d = (d + dd) & 0xffffffff;
    e = (e + ee) & 0xffffffff; f = (f + ff) & 0xffffffff;
    g = (g + gg) & 0xffffffff; h = (h + hh) & 0xffffffff;
  }

  final out = <int>[];
  for (final x in [a, b, c, d, e, f, g, h]) {
    out.add((x >> 24) & 0xff);
    out.add((x >> 16) & 0xff);
    out.add((x >> 8) & 0xff);
    out.add(x & 0xff);
  }
  return Uint8List.fromList(out);
}

/// 返回十六进制小写字符串
String sha256Hex(String input) {
  final bytes = sha256Bytes(utf8.encode(input));
  return bytes.map((b) => b.toRadixString(16).padLeft(2, '0')).join();
}
